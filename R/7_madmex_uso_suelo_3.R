...
for (region in c_list){
    ...
    grs <- madmx_points[madmx_points$layer==27,]
    if (nrow(grs)>0){
    ...
    ars <- madmx_points[madmx_points$layer==28,]
    if (nrow(ars)>0){
    ...
    urs <- madmx_points[madmx_points$layer==29,]
    if (nrow(urs)>0){
...
saveRDS(full_df, "./data_features/7_madmex_landuse_3_1_10_12.rds")
